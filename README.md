...
CheckMate 1.8.3 currently writes a JSON summary containing only red/yellow finding details; green/passed validators are filtered out before serialization. The assistant therefore prints `Passed checks: 0` plus an explicit note that passed validators were not included. It does not infer a total or invent pass results.
